...
def afficher_cadres(cadres)
  ...
  #  Helper function to display the formatted output (general)
  def afficher_simulation(chaine_reference, cadres_historique, defauts_page_positions)
    puts "références de pages logiques"
    puts chaine_reference.join(" ")
  
    puts " " * chaine_reference.length * 2 # Adjust spacing
  ...
  #  Helper function to display Second Chance output
  def afficher_simulation_seconde_chance(chaine_reference, cadres_historique, defauts_page_positions)
    puts "référence de pages logiques"
    puts chaine_reference.join(" ")
  
    puts " " * chaine_reference.length * 3 # Adjust spacing
  
    puts "\npages physiques -> pages logiques"
  
    # Display the frames with reference bits
    cadres_historique.each_with_index do |cadres, index|
      print "-------" # Wider frame
    end
    puts ""
    ...
    cadres_historique.each_with_index do |cadres, index|
      max_height = cadres_historique.map { |c| c.compact.length }.max # Handle nil values
      (0...max_height).each do |i|
        print "|"
        if cadres[i]
          page, ref_bit = cadres[i]
          sign = ref_bit == 1 ? "+" : "-"
          print "#{page}#{sign}".center(5) # Wider center
        else
          print "     " # 5 spaces
        end
        print "|"
  ...
  def simuler_seconde_chance(donnees)
    ...
    chaine_reference.each_with_index do |page, index|
      ...
      cadres_historique << cadres.zip(bits_reference).map { |p, r| p.nil? ? nil : [p, r] } # Store page and ref bit
    ...
    afficher_simulation_seconde_chance(chaine_reference, cadres_historique, defauts_page_positions)
  end
  ...
  def generer_graphiques(donnees)
    puts "\n--- Génération des Graphiques avec Ruby2D ---"
  
    if donnees[:algorithm_results].empty?
      puts "Aucune simulation n'a été effectuée. Veuillez exécuter au moins une simulation avant de générer un graphique."
      return
    end
  
    # Préparer les données pour le graphique
    algorithmes = donnees[:algorithm_results].keys.map(&:to_s)
    defauts_pages = donnees[:algorithm_results].values.map { |resultat| resultat[:defauts_page] }
  
    # Définir les dimensions de la fenêtre
    largeur_fenetre = 800
    hauteur_fenetre = 600
    set width: largeur_fenetre, height: hauteur_fenetre, title: "Nombre de fautes de page par algorithme"
  
    # Dessiner les axes
    Line.new(x1: 50, y1: hauteur_fenetre - 50, x2: largeur_fenetre - 50, y2: hauteur_fenetre - 50, width: 2, color: 'orange') # Axe X
    Line.new(x1: 50, y1: hauteur_fenetre - 50, x2: 50, y2: 50, width: 2, color: 'black') # Axe Y
  
    # Dessiner les points et les lignes
    max_defauts = defauts_pages.max
    espace_x = (largeur_fenetre - 100) / (algorithmes.size - 1).to_f
    points = []
  
    defauts_pages.each_with_index do |valeur, index|
      x = 50 + index * espace_x
      y = hauteur_fenetre - 50 - (valeur.to_f / max_defauts * (hauteur_fenetre - 100))
      points << [x, y]
    ...
    # Ajouter des étiquettes pour les algorithmes
    algorithmes.each_with_index do |alg, index|
      x = 50 + index * espace_x
      Text.new(alg.upcase, x: x - 20, y: hauteur_fenetre - 40, size: 12, color: 'red')
    end
